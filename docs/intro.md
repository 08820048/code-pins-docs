...
CodePins 是一款轻量级图钉系统插件，支持将任意代码行或代码块"📌"固定为标记，并支持备注、跳转、搜索、删除、清空、导入导出、分享、排序、拖放排序、标签筛选、复制、刷新、图钉计数、多格式分享导出，社交分享，水印设置等丰富功能，适用于临时笔记、错误定位与 TODO 跟踪，提高开发效率。
...
- 基础图钉功能（添加、删除、搜索、跳转）
- 基础标签功能（每个图钉最多 3 个标签，最多创建 10 种不同标签）
- 基础备注功能
- 基础排序功能
- 基础导入导出功能
- 基础代码预览功能

### 专业版功能(部分功能尚在开发中，升级到专业版自动享受后续全部高级功能)
...
1. 安装插件（通过 JetBrains Marketplace 或通过 IDEA 中的插件入口进行搜索安装）。
2. 在任意代码中右键点击 → Add CodePin Here（或使用快捷键 Alt+Shift+P）即可添加图钉。
3. 打开左侧工具栏" CodePins"查看图钉列表。
4. 双击跳转、右键操作、顶部支持搜索与清空、拖放排序、标签筛选、排序、批量删除、导入导出、分享、复制、刷新 等。
5. 使用标签（如 #bug、#todo）组织、筛选图钉。

## 📌 图钉的添加

- 在任意代码行上，右键点击 → Add CodePin Here（（或使用快捷键 Alt+Shift+P）即可添加图钉。
- 弹出对话框，输入备注（可选）后，点击"确定"即可。  
  ![添加图钉示例](https://your-image-url-add-pin.png)  
- 支持单行代码和代码块的添加和自动识别，如果是添加代码块图钉，鼠标选中代码块右键添加即可。
...
- 通过[问题反馈](/bug-report)页面联系我们
- 发送邮件至我们的支持邮箱：ilikexff@gmail.com
